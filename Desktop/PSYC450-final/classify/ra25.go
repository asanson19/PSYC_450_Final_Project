...
type Sim struct {
	...
	//Edit
	Tp map[string]int `desc:"true positives for each class"`
	Fp map[string]int `desc:"false positives for each class"`
	Fn map[string]int `desc:"false negatives for each class"`
	Tn map[string]int `desc:"true negatives for each class"`
}
...
func (ss *Sim) ConfigNet(net *leabra.Network) {
	...
	net.InitName(net, "HogwartsHousePredictor")
	inp := net.AddLayer2D("Input", 1, 14, emer.Input)
	hid1 := net.AddLayer2D("Hidden1", 12, 12, emer.Hidden)
	hid2 := net.AddLayer2D("Hidden2", 10, 10, emer.Hidden)
	out := net.AddLayer2D("Output", 1, 4, emer.Target) // 4 Hogwarts houses
...
func (ss *Sim) ConfigPats() {
	...
	sch := etable.Schema{
		...
		{"Input", etensor.FLOAT32, []int{1, 14}, []string{"Y", "X"}},
		{"Output", etensor.FLOAT32, []int{1, 4}, []string{"Batch", "Y"}},
	}
	dt.SetFromSchema(sch, 10)

	patgen.PermutedBinaryRows(dt.Cols[1], 6, 1, 0)
	patgen.PermutedBinaryRows(dt.Cols[2], 6, 1, 0)
	dt.SaveCSV("harrypotter.tsv", etable.Tab, etable.Headers)
}
...
func (ss *Sim) OpenPats() {
	...
	dt.SetMetaData("name", "TrainPats")
	dt.SetMetaData("desc", "Training patterns")
	err := dt.OpenCSV("normalized_data.tsv", etable.Tab)
	if err != nil {
		log.Println(err)
	}
}
...
func (ss *Sim) InitStats() {
	...
	//Edit
	// Initialize maps for TP, FP, FN, TN
	ss.Tp = make(map[string]int)
	ss.Fp = make(map[string]int)
	ss.Fn = make(map[string]int)
	ss.Tn = make(map[string]int)
}

// Edit
// You need to call this at the beginning of each run
func (ss *Sim) ResetClassCounts() {
	for class := range ss.Tp {
		ss.Tp[class] = 0
		ss.Fp[class] = 0
		ss.Fn[class] = 0
		ss.Tn[class] = 0
...
func (ss *Sim) TrialStats() {
	...
	// Classification params update
	//Edit
	// Assuming out.UnitVals returns a slice of activation values for the output units
	pred := make([]float32, 4)  // 4 for the number of output units
	out.UnitVals(&pred, "ActM") // Get activations as a slice

	if ss.TestEnv.Trial.Cur < 0 {
		// log.Println("Error: Current trial index is out of range")
		return
	}

	stateTensor := ss.TestEnv.State("Output")
	if stateTensor == nil {
		// Handle the error: State should not be nil at this point
		log.Println("Error: State tensor for 'Output' is nil")
		return
	}
	actual := stateTensor.(*etensor.Float32).Values

	// stateTensor, ok := ss.TestEnv.State("Output").(*etensor.Float32)
	// if !ok {
	// 	// Handle the error: Incorrect type
	// 	log.Println("Error: State tensor for 'Output' is not of type *etensor.Float32")
	// 	return
	// }
	// actual := stateTensor.Values

	// Convert activations to predicted class index and actual class index
	predIdx := argmax(pred) // argmax returns the index of the max value
	actualIdx := argmax(actual)

	// Increment the Tp, Fp, Fn, Tn counters based on predicted and actual classes
	// The following assumes that you have a method called UpdateClassCounters
	ss.UpdateClassCounters(predIdx, actualIdx)
...
// Edit
func (ss *Sim) UpdateClassCounters(predIdx, actualIdx int) {
	classes := []string{"Gryffindor", "Hufflepuff", "Slytherin", "Ravenclaw"}

	if predIdx == actualIdx {
		// True Positive for predicted class
		ss.Tp[classes[predIdx]]++
		// True Negative for other classes
		...
		ss.Fn[classes[actualIdx]]++
		// False Positive for predicted class
		ss.Fp[classes[predIdx]]++
		// True Negative for other classes
		for i, cls := range classes {
			if i != predIdx && i != actualIdx {
				ss.Tn[cls]++
			}
		}
	}
...
// Edit
// Then, after all trials are done, compute the metrics for each class
func (ss *Sim) ComputeMetrics() {
	// Maps to hold the computed metrics
	...
	for class := range ss.Tp {
		...
		accuracy[class] = float64(tp+tn) / float64(tp+fp+fn+tn)
		if tp+fp > 0 {
		...
		fmt.Printf("Class: %s, Accuracy: %.3f, Precision: %.3f, Recall: %.3f\n", class, accuracy[class], precision[class], recall[class])
	}
	ss.ResetClassCounts()

	// You could save or log these metrics as needed
}
